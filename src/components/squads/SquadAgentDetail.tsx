    ...
    <div className="h-full overflow-y-auto">
      <div className="p-6">
        ...
        >
          <div className="flex items-start gap-4">
            ...
            <div className="flex-1 min-w-0">
              ...
              {agent.title && (
                <p className="text-label text-[var(--accent-gold)] mt-0.5">
                  {agent.title}
                </p>
              )}
              ...
              {/* Description */}
              {agent.description && (
                <p className="text-label text-[var(--text-muted)] mt-2 leading-relaxed">
                  {agent.description}
                </p>
              )}
              ...
              {/* Tier badge */}
              {agent.tier && (
                <span className="inline-block mt-2 text-caption uppercase tracking-wider px-2 py-0.5 border border-[var(--border)] text-[var(--text-muted)]">
                  {agent.tier}
                </span>
              )}
            </div>
          ...
          {agent.tools.length > 0 && (
            <div className="mt-4 pt-3 border-t border-[var(--border-subtle)]">
              <span className="text-caption font-medium uppercase tracking-wider text-[var(--text-muted)] block mb-2">
                Ferramentas
              </span>
              <div className="flex flex-wrap gap-1.5">
                {agent.tools.map((tool, i) => (
                  <span
                    key={i}
                    className="text-detail px-2 py-0.5 bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--text-secondary)]"
                  >
                    {tool}
                  </span>
                ))}
        ...
        {viewMode === 'source' && agent.sourceMarkdown ? (
          <div className="mb-8">
            {agent.sourcePath && (
              <div className="text-caption text-[var(--text-muted)] font-mono mb-3">
                {agent.sourcePath}
              </div>
            )}
              ...
              <div className="mb-8">
                ...
                <ul className="space-y-1 ml-1">
                  {agent.principles.map((p, i) => (
                    <li key={i} className="text-label text-[var(--text-secondary)] leading-relaxed flex items-start gap-2">
              ...
              <div className="mb-8">
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-1.5">
                  {agent.commands.map((cmd, i) => (
                    <div
                      key={i}
                      className="text-detail font-mono px-2 py-1 bg-[var(--bg-elevated)] border border-[var(--border)] text-[var(--text-secondary)] truncate"
                    >
                      {cmd}
                    </div>
                  ))}
              ...
              <div className="mb-8">
                ...
                <div className="space-y-1.5">
                  {agent.handoffs.map((h, i) => (
                    <div
                      key={i}
                      className="text-label flex items-center gap-2 text-[var(--text-secondary)]"
                    >
                      <span className="font-mono text-[var(--accent-gold)]">{h.agent}</span>
                      <span className="text-[var(--text-muted)]">-</span>
                      <span className="text-[var(--text-muted)]">{h.when}</span>
                      {h.squad && (
                        <span className="text-caption px-1.5 py-0.5 border border-[var(--border)] text-[var(--text-muted)]">
                          {h.squad}
                        </span>
                      )}
                    </div>
